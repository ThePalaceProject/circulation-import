...
class SFTPClient:
    ...
    def __exit__(
        ...
        if self._client_instance:
            self._logger.debug(
                "Started closing SFTP session and its underlying channel"
            )

            self._client_instance.close()

            self._logger.debug(
                "Finished closing SFTP session and its underlying channel"
            )
        if self._transport:
    ...
    def _client(self) -> paramiko.SFTPClient:
        if self._client_instance is None:
            ...
            self._transport.connect(
                None, self._configuration.username, self._configuration.password
            )
    ...
    def upload(
        self, local_file_path: str, remote_file_path: Optional[str] = None
    ) -> None:
        self._logger.debug(
            f"Started uploading local file {local_file_path} to {remote_file_path}"
        )

        if not remote_file_path:
            file_name = os.path.basename(local_file_path)
            remote_file_path = os.path.join(
                self._configuration.upload_directory, file_name
            )

            self._logger.debug(
                f"There is no remote file path specified. New file remote file path: {remote_file_path}"
            )

        self._client.put(local_file_path, remote_file_path)

        self._logger.debug(
            f"Finished uploading local file {local_file_path} to {remote_file_path}"
        )

    def download(self, remote_file_path: str, local_file_path: str) -> None:
        self._logger.debug(
            f"Started downloading {remote_file_path} to {local_file_path}"
        )

        self._client.get(remote_file_path, local_file_path)

        self._logger.debug(
            f"Finished downloading {remote_file_path} to {local_file_path}"
        )
