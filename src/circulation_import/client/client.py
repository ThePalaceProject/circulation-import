...
from circulation_import.client.storage import ClientStorage
...
class Client:
    def __init__(
        ...
        report_serializer: CollectionImportMetadataSerializer,
    ) -> None:
        self._configuration: ClientConfiguration = configuration
        self._storage: ClientStorage = storage
    ...
    def _upload_books(
        ...
        local_books_directory: str,
        remote_books_directory: str,
        import_content: bool,
    ) -> List[BookImportMetadata]:
        self._logger.info(
            f"Started uploading books from {local_books_directory} to {remote_books_directory}"
        )
        ...
        for (directory, subdirectories, book_files) in os.walk(local_books_directory):
            for book_file in book_files:
                if not self._has_extension(
                    book_file, self._configuration.book_file_types
                ):
                    continue

                local_book_file_path = os.path.join(directory, book_file)
                book_import_metadata = BookImportMetadata(
                    name=book_file,
                    hash=utils.calculate_file_hash(
                        local_book_file_path, self._configuration.hashing_algorithm
                    ),
                )

                if self._storage.find_book(collection_metadata, book_import_metadata):
                    self._logger.debug(
                        f"Book {book_import_metadata} has been already processed"
                    )
                else:
                    books_metadata.append(book_import_metadata)

                    if import_content:
                        remote_book_file_path = os.path.join(
                            remote_books_directory, book_file
                        )

                        self._sftp_client.upload(
                            local_book_file_path, remote_book_file_path
                        )
    ...
    def _upload_covers(
        self, local_covers_directory: str, remote_covers_directory: str
    ) -> None:
        self._logger.info(
            f"Started uploading covers from {local_covers_directory} to {remote_covers_directory}"
        )
        ...
        for (directory, subdirectories, cover_files) in os.walk(local_covers_directory):
            for cover_file in cover_files:
                if not self._has_extension(
                    cover_file, self._configuration.cover_file_types
                ):
                    continue
                ...
                remote_cover_file_path = os.path.join(
                    remote_covers_directory, cover_file
                )
    ...
    def _upload_metadata_file(
        self, local_metadata_file: str, remote_metadata_file: str
    ) -> None:
        self._logger.info(
            f"Started uploading metadata file {local_metadata_file} to {remote_metadata_file}"
        )

        self._sftp_client.upload(local_metadata_file, remote_metadata_file)

        self._logger.info(
            f"Finished uploading metadata file {local_metadata_file} to {remote_metadata_file}"
        )

    def _upload_import_metadata_file(
        self,
        collection_import_metadata: CollectionImportMetadata,
        remote_import_metadata_file: str,
    ) -> None:
        self._logger.info(
    ...
    def _download_report(self, remote_report_path: str) -> CollectionImportMetadata:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            local_report_path = os.path.join(
                temporary_directory, MetadataFile.REPORT_FILE.value
            )
    ...
    def _save_report_locally(
        self, local_reports_directory: str, report: CollectionImportMetadata
    ) -> str:
        ...
        public_report_path = (
            os.path.join(local_reports_directory, "report")
            + date_time_suffix
            + self._report_serializer.extension
        )
    ...
    def _wait_for_report(
        self,
        collection_import_metadata: CollectionImportMetadata,
        remote_reports_directory: str,
    ) -> Tuple[CollectionImportMetadata, str]:
        self._logger.info(f"Started waiting for a report in {remote_reports_directory}")

        if not os.path.exists(collection_import_metadata.reports_directory):
            os.makedirs(collection_import_metadata.reports_directory)
        ...
        while True:
            ...
            self._logger.info(
                f"Found the following files in {remote_reports_directory}: {files}"
            )

            for file in files:
                if MetadataFile.REPORT_FILE.value in file:
                    remote_report_path = os.path.join(
                        remote_reports_directory, MetadataFile.REPORT_FILE.value
                    )

                    report = self._download_report(remote_report_path)
                    public_report_path = self._save_report_locally(
                        collection_import_metadata.reports_directory, report
                    )
    ...
    def _print_result(
        self, report: CollectionImportMetadata, public_report_path: str
    ) -> None:
        failed = any(
            [book.status == ProcessingStatus.FAILED.value for book in report.books]
        )

        if failed:
            self._logger.error(
                f"Import failed. Please refer to {public_report_path} for more details"
            )
        else:
    ...
    def run(
        ...
        self._logger.info(
            f"Started importing a collection using the following configuration: {collection_import_metadata}"
        )

        collection_directory = os.path.join(
            self._configuration.sftp_configuration.upload_directory,
            collection_import_metadata.collection_name,
            collection_import_metadata.data_source_name,
        )

        remote_books_directory = os.path.join(
            collection_directory, Directory.BOOKS_DIRECTORY.value
        )
        self._create_remote_directory(remote_books_directory)

        remote_covers_directory = os.path.join(
            collection_directory, Directory.COVERS_DIRECTORY.value
        )
        self._create_remote_directory(remote_covers_directory)

        remote_reports_directory = os.path.join(
            collection_directory, Directory.REPORTS_DIRECTORY.value
        )
        self._create_remote_directory(remote_reports_directory)

        remote_metadata_file = os.path.join(
            collection_directory, MetadataFile.COLLECTION_METADATA_FILE.value
        )
        remote_import_metadata_file = os.path.join(
            collection_directory, MetadataFile.IMPORT_METADATA_FILE.value
        )
        ...
        if collection_metadata is not None:
            self._logger.info(
                f"Collection {collection_import_metadata} already exists: {collection_metadata}"
            )
        else:
            self._logger.info(
                f"Collection {collection_import_metadata} does not exist, started creating a new one"
            )
            ...
            collection_metadata = CollectionMetadata(
                ...
                books_directory=collection_import_metadata.books_directory,
                covers_directory=collection_import_metadata.covers_directory,
                reports_directory=collection_import_metadata.reports_directory,
                metadata_file=collection_import_metadata.metadata_file,
            ...
            self._logger.info(
                f"Finished creating a new collection: {collection_metadata}"
            )

        books = self._upload_books(
            collection_metadata,
            collection_import_metadata.books_directory,
            remote_books_directory,
            self._configuration.import_content,
        )

        if self._configuration.import_content:
            self._upload_covers(
                collection_import_metadata.covers_directory, remote_covers_directory
            )

        self._upload_metadata_file(
            collection_import_metadata.metadata_file, remote_metadata_file
        )
        ...
        self._upload_import_metadata_file(
            collection_import_metadata, remote_import_metadata_file
        )

        report, public_report_path = self._wait_for_report(
            collection_import_metadata, remote_reports_directory
        )
