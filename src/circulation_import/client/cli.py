...
from circulation_import.metadata.serialization import (
    CollectionImportMetadataCSVSerializer,
)
...
def _load_client_configuration(client_configuration_file: str) -> ClientConfiguration:
    logging.info(
        f"Started loading client's configuration from {client_configuration_file}"
    )

    client_configuration = ClientConfiguration.load(client_configuration_file)

    logging.info(
        f"Finished loading client's configuration from {client_configuration_file}"
    )
...
@click.option(
    "--collection-name", "-c", help="Name of the collection", required=True, type=str
)
@click.option(
    "--data-source-name", "-d", help="Name of the data source", required=True, type=str
)
@click.option(
    "--books-directory",
    "-bd",
...
@click.option(
    ...
    required=True,
    type=click.Choice(
        list(map(lambda x: x.value, MetadataFormat)), case_sensitive=False
    ),
)
@click.option(
    "--configuration-file",
...
def run_import(
    ...
    session_factory = DbSessionFactory[ClientStorage](
        client_configuration.database_configuration
    )

    with session_factory.create() as storage:
        client_instance = Client(
            client_configuration, storage, sftp_client, report_serializer
        )
        collection_configuration = CollectionImportMetadata(
