...
class CollectionImportMetadataSerializer(ABC):
    ...
    @abstractmethod
    def serialize(
        self, collection_import_metadata: CollectionImportMetadata, file_path: str
    ) -> None:
        raise NotImplementedError()
...
class CollectionImportMetadataCSVSerializer(CollectionImportMetadataSerializer):
    ...
    def __init__(
        self, delimiter: str = DEFAULT_DELIMITER, quote_char: str = DEFAULT_QUOTE_CHAR
    ) -> None:
    ...
    def serialize(
        ...
        self._logger.info(
            f"Started serializing {collection_import_metadata} to {file_path}"
        )

        with open(file_path, "w") as output_file:
            writer = csv.writer(
                output_file, delimiter=self._delimiter, quotechar=self._quote_char
            )
        ...
        self._logger.info(
            f"Finished serializing {collection_import_metadata} to {file_path}"
        )
