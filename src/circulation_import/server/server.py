...
class Server:
    def __init__(
        ...
        self._file_in_progress: Optional[str] = None
    ...
    def _get_local_path(self, directory: str, file_name: Optional[str] = None) -> str:
        ...
        if common_directory in directory or (
            file_name and common_directory in file_name
        ):
            base_directory = base_directory.rstrip(common_directory)

        local_path = os.path.join(base_directory, directory)
    ...
    def _process_import_metadata(
        self, collection_import_metadata: CollectionImportMetadata
    ) -> CollectionMetadata:
        ...
        if collection_metadata is not None:
            ...
            local_books_directory = self._get_local_path(
                collection_import_metadata.books_directory
            )
            local_covers_directory = self._get_local_path(
                collection_import_metadata.covers_directory
            )
            local_reports_directory = self._get_local_path(
                collection_import_metadata.reports_directory
            )
            local_metadata_file = self._get_local_path(
                "", collection_import_metadata.metadata_file
            )
        ...
        for book_import_metadata in collection_import_metadata.books:
            ...
            book_metadata = self._storage.find_book(
                collection_metadata, book_import_metadata
            )

            if book_metadata is not None:
                self._logger.debug(
                    f"Book {book_import_metadata} already exists: {book_metadata}"
                )
            else:
                self._logger.debug(
                    f"Book {book_import_metadata} does not exist, started creating a new one"
                )
            ...
            if book_metadata.status not in [
                ...
                book_hash = utils.calculate_file_hash(
            ...
            self._logger.debug(
                f"Finished processing {book_import_metadata}: {book_metadata}"
            )

        self._logger.info(
            f"Finished processing {collection_import_metadata}: {collection_metadata}"
        )
    ...
    def _save_report(
        ...
        self._logger.info(
            f"Finished saving report for {collection_metadata} to {local_report_file_path}"
        )

    def _import_collection(self, import_metadata_file: str) -> None:
        self._logger.info(
            f"Started importing the collection from {import_metadata_file}"
        )

        # We have to use a critical section in the case of SQLite because it doesn't allow
        # multi-threaded access to the database
        if (
            self._configuration.database_configuration.driver
            == DatabaseDriver.SQLITE.value
        ):
            self._lock.acquire()

        try:
            collection_import_metadata = CollectionImportMetadata.load(
                import_metadata_file
            )

            collection_metadata = self._process_import_metadata(
                collection_import_metadata
            )
            ...
            self._logger.exception(
                "An unexpected exception occurred during during collection import"
            )
        ...
        self._logger.info(
            f"Finished importing the collection from {import_metadata_file}"
        )
    ...
    def _process_file_system_event(self, event: FileSystemEvent) -> None:
        ...
        if file_name == self._file_in_progress:
            self._logger.debug(f"{file_name} is already being processed")
            return

        if file_name == MetadataFile.IMPORT_METADATA_FILE.value:
            time.sleep(10)  # Wait 10 seconds to make sure that the file is available

            self._import_collection(event.src_path)

            self._file_in_progress = None
    ...
    def run(self) -> None:
        ...
        observer = Observer()
        observer.schedule(
            event_handler, self._configuration.upload_directory, recursive=True
        )
        observer.start()
