...
def _load_server_configuration(server_configuration_file: str) -> ServerConfiguration:
    logging.info(
        f"Started loading server's configuration from {server_configuration_file}"
    )

    server_configuration = ServerConfiguration.load(server_configuration_file)

    logging.info(
        f"Finished loading server's configuration from {server_configuration_file}"
    )
...
def monitor(configuration_file: str, logging_configuration_file: str) -> None:
    ...
    session_factory = DbSessionFactory[ServerStorage](
        server_configuration.database_configuration
    )
