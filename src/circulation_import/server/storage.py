...
class ServerStorage(DbStorage):
    def find_collection(
        ...
        existing_collection = (
            self.query(CollectionMetadata)
            .filter(
                CollectionMetadata.name == collection_import_metadata.collection_name
            )
            .filter(
                CollectionMetadata.data_source_name
    ...
    def find_book(
        self, collection: CollectionMetadata, book_import_metadata: BookImportMetadata
    ) -> BookMetadata:
        existing_book = (
            self.query(BookMetadata)
